...
function renderProduct(x) {
  ...
  <div class="p-4 w-full md:w-1/2 lg:w-1/3">
    <div class="border h-full p-4 flex flex-col">
      <h2 class="text-lg font-bold mb-4">${x.name}</h2>
      <div class="mb-4"><img src="${x.image}">
      </div>
      <div class="mb-4 text-gray-900">
      ${x.description}
      </div>
      <div class="mt-auto text-purple-500 text-2xl">$${x.price}</div>
    </div>
  </div>  
`)
}
...
async function pageLoaded() {
  ...
  for (i=0; i<json.products.length; i++) {
    console.log(`product is: ${json.products[i].name}`)
